...
This repository contains a static campus and an independent legacy Next.js application. The campus is generated from teaching sources into `campus/dist`. It does not need the legacy application's dependencies, OpenNext, an R2 cache or a self-reference service binding. A root-level generic Wrangler command without the intended configuration can select the wrong application.
