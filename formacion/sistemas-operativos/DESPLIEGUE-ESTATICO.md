...
El repositorio contiene dos aplicaciones independientes. `campus/` transforma el temario Markdown y el catálogo curricular en HTML, CSS, JavaScript y JSON. El navegador presenta el contenido y conserva el progreso local. La aplicación Next.js de `src/` es un ejemplo distinto: su directorio `.next` no es la salida del curso.
